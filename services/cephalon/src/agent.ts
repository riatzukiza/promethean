...
export class AIAgent extends EventEmitter {
    ...
    generateJSONResponse(specialQuery:string, {
        ...
        format,
        prompt = this.prompt
    }: GenerateResponseOptions ):Promise<object> {
    ...
    generateTextResponse(specialQuery:string, {
        context,
        prompt = this.prompt
...
,
    ...
    async generateVoiceResponse() {
        try {
            ...
            if (!content) {
                content = "I'm a duck, who's name is Duck. How creative. Quack quack quack. Seems like there is a problem with my AI."
            }
